...
class InferenceOCRClient:
    ...
    def detect_diagram(self, image_path, model_id):
        ...
        detection_result_objects = detection_client.infer(image, model_id=model_id)
        ...
        for idx, prediction in enumerate(detection_result_objects["predictions"]):
            ...
            output_image_path = f'static/detected_images/cropped_image_{idx}.jpg'
...
def upload_image():
    ...
    if file:
        ...
        detection_result = OCR_CLIENT.detect_diagram(image_path, model_id=model_id)
...
def draw_bounding_boxes(image_path, detections):
    ...
    for detection in detections:
        ...
        if detection['text']:
            draw.text((x, y), detection['text'], fill="red")
